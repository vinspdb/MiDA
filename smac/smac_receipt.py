...
def get_model(cfg):
    ...
    layer_in = concatenate(([diff2, x_act, x_res, x_att1, x_att2, x_att3, x_att4, x_att5]))
    ...
    model = Model(inputs=[input_diff2, input_act, input_res, input_att1, input_att2, input_att3, input_att4, input_att5], outputs=out)
...
def fit_and_score(cfg):
    print(cfg)
    outfile2 = open("smac_receipt" + str(f) + "batch.txt", 'a')
    ...
    h = model.fit(
        [diff2_train, sequence_train, resource_train, att1_train, att2_train, att3_train, att4_train, att5_train],
    ...
    if best_score > score:
        ...
        best_model.save("models/receiptmodel_smac_" + str(f) + "_batch.h5")
...
for f in range(3):
    ...
    logger = logging.getLogger("receipt_fold_" + str(f))
    ...
    batch_size = CategoricalHyperparameter("batch_size", [32, 64, 128], default_value=32)
