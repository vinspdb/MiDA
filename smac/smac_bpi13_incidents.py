...
def get_model(cfg):
    ...
    size_att7 = (num_att7 + 1) // 2
    input_att7 = Input(shape=(lenght_seq,), dtype='int32', name='input_att7')
    x_att7 = Embedding(output_dim=size_att7, input_dim=num_att7+1, input_length=lenght_seq)(input_att7)

    layer_in = concatenate(([diff2, x_act, x_res, x_att1, x_att2, x_att3, x_att4, x_att5, x_att6, x_att7]))
    ...
    opt = Nadam(lr=cfg['learning_rate_init'], beta_1=0.9, beta_2=0.999, epsilon=1e-08, schedule_decay=0.004,
                clipvalue=3)
    model = Model(inputs=[input_diff2, input_act, input_res, input_att1, input_att2, input_att3, input_att4, input_att5, input_att6, input_att7], outputs=out)
...
def fit_and_score(cfg):
    print(cfg)
    outfile2 = open("smac_bpi13_incidents" + str(f) + ".txt", 'a')
    ...
    h = model.fit([diff2_train, sequence_train, resource_train, att1_train, att2_train, att3_train, att4_train, att5_train, att6_train, att7_train], Y_train, epochs=200, verbose=0, validation_split=0.2, callbacks=[early_stopping, lr_reducer],batch_size=cfg['batch_size'])
    ...
    if best_score > score:
        ...
        best_model.save("models/bpi13_incidentsmodel_smac_"+str(f)+"_batch.h5")
...
info_dataset = np.load("fold/bpi13_incidents/" + str(benchmark) + "_info_dataset.npy")
...
for f in range(3):
    ...
    logger = logging.getLogger("bpi13_problems_fold_ATR" + str(f))
    ...
    batch_size = CategoricalHyperparameter("batch_size", [256, 512], default_value=256)
    ...
    print("FOLD numero----------------------->", f)
    print("<--------------------------------->")
    sequence_train = np.load("fold/bpi13_incidents/" + str(benchmark) + "_act_" + str(f) + "_train.npy")
    resource_train = np.load("fold/bpi13_incidents/" + str(benchmark) + "_res_" + str(f) + "_train.npy")
    diff2_train = np.load("fold/bpi13_incidents/" + str(benchmark) + "_diff2_" + str(f) + "_train.npy")
    att1_train = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att1_" + str(f) + "_train.npy")
    att2_train = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att2_" + str(f) + "_train.npy")
    att3_train = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att3_" + str(f) + "_train.npy")
    att4_train = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att4_" + str(f) + "_train.npy")
    att5_train = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att5_" + str(f) + "_train.npy")
    att6_train = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att6_" + str(f) + "_train.npy")
    att7_train = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att7_" + str(f) + "_train.npy")

    y_train = np.load("fold/bpi13_incidents/" + str(benchmark) + "_y_" + str(f) + "_train.npy")

    # test views
    sequence_test = np.load("fold/bpi13_incidents/" + str(benchmark) + "_act_" + str(f) + "_test.npy")
    resource_test = np.load("fold/bpi13_incidents/" + str(benchmark) + "_res_" + str(f) + "_test.npy")
    diff2_test = np.load("fold/bpi13_incidents/" + str(benchmark) + "_diff2_" + str(f) + "_test.npy")
    att1_test = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att1_" + str(f) + "_test.npy")
    att2_test = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att2_" + str(f) + "_test.npy")
    att3_test = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att3_" + str(f) + "_test.npy")
    att4_test = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att4_" + str(f) + "_test.npy")
    att5_test = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att5_" + str(f) + "_test.npy")
    att6_test = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att6_" + str(f) + "_test.npy")
    att7_test = np.load("fold/bpi13_incidents/" + str(benchmark) + "_att7_" + str(f) + "_test.npy")
    y_test = np.load("fold/bpi13_incidents/" + str(benchmark) + "_y_" + str(f) + "_test.npy")
