...
def fit_and_score(cfg):
    print(cfg)
    outfile2 = open("smac_bpi12w_complete" + str(f) + ".txt", 'a')
    ...
    if best_score > score:
        ...
        best_model.save("models/bpi12wcompletemodel_smac_"+str(f)+"_batch.h5")
...
info_dataset = np.load("fold/bpi12w_complete/" + str(benchmark) + "_info_dataset.npy")
...
for f in range(3):
    ...
    logger = logging.getLogger("bpi12w_complete_fold_" + str(f))
    ...
    batch_size = CategoricalHyperparameter("batch_size", [256, 512, 1024], default_value=512)
    ...
    print("FOLD numero----------------------->", f)
    print("<--------------------------------->")
    sequence_train = np.load("fold/bpi12w_complete/" + str(benchmark) + "_act_" + str(f) + "_train.npy")
    resource_train = np.load("fold/bpi12w_complete/" + str(benchmark) + "_res_" + str(f) + "_train.npy")
    diff2_train = np.load("fold/bpi12w_complete/" + str(benchmark) + "_diff2_" + str(f) + "_train.npy")
    qt_train = np.load("fold/bpi12w_complete/" + str(benchmark) + "_amount_" + str(f) + "_train.npy")
    y_train = np.load("fold/bpi12w_complete/" + str(benchmark) + "_y_" + str(f) + "_train.npy")

    # test views
    sequence_test = np.load("fold/bpi12w_complete/" + str(benchmark) + "_act_" + str(f) + "_test.npy")
    resource_test = np.load("fold/bpi12w_complete/" + str(benchmark) + "_res_" + str(f) + "_test.npy")
    diff2_test = np.load("fold/bpi12w_complete/" + str(benchmark) + "_diff2_" + str(f) + "_test.npy")
    qt_test = np.load("fold/bpi12w_complete/" + str(benchmark) + "_amount_" + str(f) + "_test.npy")
    y_test = np.load("fold/bpi12w_complete/" + str(benchmark) + "_y_" + str(f) + "_test.npy")
