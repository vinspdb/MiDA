...
def get_model(cfg):
    ...
    layer_l = LSTM(units=int(cfg["lstmsize1"]), implementation=2, kernel_initializer='glorot_uniform',
                   return_sequences=True, dropout=0.2)(layer_in)
    ...
    layer_l = LSTM(units=int(cfg["lstmsize2"]), implementation=2, kernel_initializer='glorot_uniform',
                   return_sequences=False, dropout=0.2)(layer_l)
    layer_l = BatchNormalization()(layer_l)
...
def fit_and_score(cfg):
    print(cfg)
    outfile2 = open("smac_bpi12_work_all" + str(f) + ".txt", 'a')
    ...
    if best_score > score:
        ...
        best_model.save("models/bpi12_work_allmodel_smac_"+str(f)+"_batch.h5")
